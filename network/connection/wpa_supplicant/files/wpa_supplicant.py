...
def detectWpaDriver(ifname):
    import os
    import sys
    import sulin.netutils
    # if device is an ethernet device, driver is "wired"
    device = sulin.netutils.IF(ifname)
...
def checkServiceState(serviceName):
    bus = dbus.SystemBus()
    obj = bus.get_object("tr.org.sulin.scom", "/package/%s" % serviceName)
    state = obj.info(dbus_interface="tr.org.sulin.scom.System.Service")[2]
...
def startWpaService():
    if not isWpaServiceActive():
        bus = dbus.SystemBus()
        obj = bus.get_object("tr.org.sulin.scom", "/package/wpa_supplicant")
        obj.start(dbus_interface="tr.org.sulin.scom.System.Service")
    import time
    timeout = 10
    while timeout > 0:
        try:
...
def disableAuthentication(ifname):
    ...
    bus = dbus.SystemBus()
    obj = bus.get_object("tr.org.sulin.scom", "/package/wpa_supplicant")
    obj.stop(dbus_interface="tr.org.sulin.scom.System.Service")
